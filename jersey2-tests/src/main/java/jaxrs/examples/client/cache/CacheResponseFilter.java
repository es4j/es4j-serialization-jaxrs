...
public class CacheResponseFilter implements ResponseFilter {
    ...
    @Override
    public void postFilter(FilterContext request) throws IOException {
        if (enabledFlag.get()) {
            store(request, request);
        }
    }

    private void store(FilterContext request, FilterContext response) {
...
/*
            ...
            CacheEntry cacheEntry = new CacheEntry(
                    ...
                    body);
            cacheStore.put(request.getRequest().getUri().toString(), cacheEntry);

            response.getResponse().setEntityStream(new ByteArrayInputStream(cacheEntry.getBody()));
*/
        }
    }
